...
5. 创建 Azure AI Services 资源，默认区域是 `eastus2`。
6. 创建 Ubuntu 24.04 LTS 虚拟机，默认规格是 `Standard_D2as_v6`。
7. 给 VM 分配 Managed Identity，并授予访问 Azure AI 资源所需的角色。
8. 通过 cloud-init 在 VM 中安装 Docker。
9. 生成 LiteLLM 配置，并以 Docker 容器方式启动 LiteLLM。
10. 创建 `litellm` systemd 服务，确保机器重启后 LiteLLM 还能自动拉起。
11. 安装 Homebrew 和 `gcc`。
12. 使用 OpenClaw 官方安装脚本安装 OpenClaw。
13. 生成统一的环境变量文件，路径是 `/home/openclaw/.openclaw/gateway.env`。
14. 生成 OpenClaw 配置文件和 LiteLLM 配置文件。
15. 输出最终登录信息、IP、SSH 命令和 VM 内的关键文件位置。
